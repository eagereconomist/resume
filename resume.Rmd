...
if (exists("certifications") && is.data.frame(certifications)) {
  for (i in seq_len(nrow(certifications))) {
    ...
    title <- c$area %||% "Untitled Certification"
    org <- c$accomplishment %||% "Unknown Issuer"
    date <- paste(c$endMonth %||% "", c$year %||% "")
    id <- c$detail

    # Don't double up Credential ID prefix
    if (!is.na(id) && nzchar(id) && !grepl("^Credential ID", id)) {
      id <- paste("Credential ID:", id)
    }

    # Title on its own line
    cat(glue::glue("<div class='cert-title'>{title}</div>\n"))

    # Org + Date (flex aligned)
    cat(glue::glue(
